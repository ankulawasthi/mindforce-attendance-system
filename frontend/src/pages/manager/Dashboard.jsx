...
export default function ManagerDashboard() {
  ...
  const [leaves, setLeaves] = useState([]) // Leaves of subordinates
  const [breakSummary, setBreakSummary] = useState([])
  const [myLeaves, setMyLeaves] = useState([]) // Manager's own leaves
  ...
  const fetchMyLeaves = async () => {
    ...
    try {
      ...
      // Logic: Pending leaves for manager to approve (others in department)
  ...
  // --- SUBORDINATE LEAVE APPROVAL ---
  const handleActionSubordinateLeave = async (id, action) => {
    try {
      await api.patch(`/leave_requests/${id}/${action}`)
      showMsg(`✅ Leave successfully ${action === 'approve' ? 'approved' : 'rejected'}`)
      fetchMyLeaves() // Refresh leave lists
    } catch (err) {
    ...
    <>
      ...
      <div style={styles.layout}>
        ...
        <div style={styles.main}>
                  ...
                  <BarChart data={chartData}>
                    ...
                    <Bar dataKey="present" radius={[4, 4, 0, 0]}>
                       {chartData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={entry.present === 1 ? '#10b981' : '#e2e8f0'} />
                       ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                ...
                <tbody>
                  {todayAll.map(a => (
                    <tr key={a.id} style={styles.tr}>
                      <td style={styles.td}><b>{a.user_name}</b></td>
                  ...
                  {todayAll.length === 0 && <tr><td colSpan="4" style={{textAlign:'center', padding:'2rem', color:'#94a3b8'}}>No logs for today yet.</td></tr>}
                </tbody>
              </table>
            </div>
          )}
          ...
          {activeTab === 'breaks' && (
            <div style={styles.card}>
              <h3 style={styles.cardTitle}>Live Break Monitor</h3>
              <table style={styles.table}>
                <thead>
                  <tr style={styles.thead}>
                    <th style={styles.th}>Employee</th>
                    <th style={styles.th}>Break Count</th>
                ...
                <tbody>
                  {breakSummary.map(b => (
                    <tr key={b.user_id} style={styles.tr}>
                      <td style={styles.td}>{b.user_name}</td>
                      <td style={styles.td}>{b.break_count}</td>
                      <td style={styles.td}>
                        <span style={{ 
                          ...styles.badge, 
                          background: b.on_break ? '#fef3c7' : '#f1f5f9', 
                          color: b.on_break ? '#d97706' : '#64748b' 
                        }}>
                          {b.on_break ? '☕ On Break' : '💻 Working'}
                        </span>
                      </td>
                    </tr>
                  ))}
                      ...
                      <tbody>
                        {leaves.map(req => (
                          <tr key={req.id} style={styles.tr}>
                            <td style={styles.td}>{req.user_name}</td>
                            <td style={styles.td}>{req.leave_type}</td>
                            <td style={styles.td}>{req.from_date} to {req.to_date}</td>
                            <td style={styles.td}>
                              <div style={{display:'flex', gap:'5px'}}>
                                <button style={{...styles.addBtn, background:'#10b981'}} onClick={() => handleActionSubordinateLeave(req.id, 'approve')}>Approve</button>
                                <button style={{...styles.addBtn, background:'#ef4444'}} onClick={() => handleActionSubordinateLeave(req.id, 'reject')}>Reject</button>
                              </div>
                            </td>
                          </tr>
                        ))}
                        {leaves.length === 0 && <tr><td colSpan="4" style={{textAlign:'center', padding:'20px', color:'#94a3b8'}}>No pending approvals.</td></tr>}
                      </tbody>
                   </table>
                </div>
              )}
